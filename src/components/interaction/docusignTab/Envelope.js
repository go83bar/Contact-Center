...
const Envelope = ({envelope}) =>
{
    ...
    const localization = useSelector(state => state.localization.interaction.docusign)
    const resendSuccessMessage = useSelector(state => state.localization.toast.docusign.resendSuccess)
    const resendErrorMessage = useSelector(state => state.localization.toast.docusign.resendError)
    const dispatch = useDispatch()

    const [isResending, setIsResending] = useState(false)
    const [resendBtnLabel, setResendBtnLabel] = useState(localization.resendButtonLabel)
    ...
    const resendEnvelope = () => {
        ...
        DocumentAPI.resendDocusign(docusignParams).then((response) => {
            if (response.success) {
            ...
            <MDBBox className="d-flex backgroundColorInherit skin-border-primary f-m w-100">
                <div className='d-flex p-1 px-3 w-100'>
                    ...
                    <div className="d-flex p-2 text-left w-75">
                        ...
                        {envelope.completed_at === null && envelope.declined_at === null && envelope.voided_at === null &&
                        <span><MDBBtn color="primary"
                    ...
                    <div className="d-flex flex-column f-s justify-content-start p-2 w-25 text-right">
                        ...
                        {envelope.resent_at !== null && <span>
                                    {localization.resentAtLabel}
                            <span className="font-weight-bold">
                                        {moment.utc(envelope.resent_at).tz(timezone).format("MMM D")}
                                    </span>, {moment.utc(envelope.resent_at).tz(timezone).format("h:mm a z")}
                                </span>}
