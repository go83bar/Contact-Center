...
class LeadDocusign extends Component {
  constructor(props) {
    ...
    if (this.props.shift.docusign_templates !== undefined) {
      ...
      this.props.shift.docusign_templates
        .filter((template) => {
  ...
  sendDocusign = () => {
    ...
    DocumentAPI.sendDocusign(docusignParams)
      .then((response) => {
        if (response.success) {
          // build new envelope to put into store
          const selectedTemplate = this.props.shift.docusign_templates.find(
  ...
  render() {
    return (
      <MDBBox
        className={
          this.props.active ? "d-flex w-100 flex-column bg-white f-m" : "hidden"
        }
      >
        <div className="d-flex w-100 justify-content-end align-items-center gray-border rounded">
          <MDBSelect
            ...
            label={
              this.props.localization.interaction.docusign.templateOptionsLabel
            }
            className="w-50 mr-2"
          />
          <MDBBtn
          ...
          >
            {this.state.disabled
              ? this.props.localization.interaction.docusign.sendingButtonLabel
              : this.props.localization.interaction.docusign.sendButtonLabel}
          </MDBBtn>
        </div>
        <MDBBox className="d-flex w-100 flex-column bg-white f-m">
          <div className="d-flex flex-column p-1 px-3 gray-background gray-border mt-2 rounded">
            <span className="f-l font-weight-bold m-2">
              {this.props.localization.interaction.docusign.listTitle}
            </span>
            {console.log(this.props.lead.docusign)}
