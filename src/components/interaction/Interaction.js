...
class Interaction extends Component {
  ...
  earlyCloseWarning = (ev) => {
    ...
    const confirmationMessage =
      this.props.localization.interaction.earlyCloseWarning;
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      prevProps.twilio.interactionIncomingCallSID === "" &&
      this.props.twilio.interactionIncomingCallSID !== ""
    ) {
  ...
  answerIncoming = () => {
    ...
    if (this.props.twilio.conferenceOID === "") {
      ...
      TwilioDevice.connectIncoming(
        this.props.twilio.interactionIncomingCallSID,
        this.props.twilio.conferenceOID
      );
    }
  };
  ...
  render() {
    ...
    const docusignVisible = this.props.shift.docusign_templates.some(
      (template) => {
    ...
    return (
      <MDBBox className="d-flex w-100 skin-secondary-color">
        <MDBBox
          className="m-0 my-2 ml-2 border rounded skin-secondary-background-color"
          style={{
            flex: slim ? "0 0 50px" : "0 0 100px",
            order: 0,
            fontSize: "14px",
            overflowY: "auto",
          }}
        >
          <MDBNav>
            ...
            <SideNavItem
              ...
              icon={faUser}
              label={localization.details.tabTitle}
              slim={slim}
            ...
            <SideNavItem
              ...
              icon={faPoll}
              label={localization.survey.tabTitle}
              rotation={90}
              slim={slim}
              onClick={this.toggleTab("surveys")}
            />
            {questionsVisible && (
              <SideNavItem
                active={this.state.activeItem === "questions"}
                icon={faPoll}
                label={localization.questions.tabTitle}
                rotation={90}
                slim={slim}
            ...
            <SideNavItem
              active={this.state.activeItem === "timeline"}
              icon={faStream}
              label={localization.timeline.tabTitle}
              slim={slim}
              onClick={this.toggleTab("timeline")}
            />
            <SideNavItem
              active={this.state.activeItem === "appointments"}
              icon={faCalendarCheck}
              label={localization.appointment.tabTitle}
              slim={slim}
            ...
            <SideNavItem
              ...
              icon={faEdit}
              label={localization.notes.tabTitle}
              slim={slim}
            ...
            <SideNavItem
              ...
              icon={faFile}
              label={localization.documents.tabTitle}
              slim={slim}
              onClick={this.toggleTab("documents")}
            />
            {docusignVisible && (
              <SideNavItem
                active={this.state.activeItem === "esignatures"}
                icon={faFileSignature}
                label={localization.docusign.tabTitle}
                slim={slim}
                onClick={this.toggleTab("esignatures")}
              />
            )}
            {rewardsVisible && (
              <SideNavItem
                ...
                icon={faGift}
                label={localization.rewards.tabTitle}
                slim={slim}
            ...
            {true && (
              <SideNavItem
                ...
                label={"Lab Orders"}
                slim={slim}
                onClick={this.toggleTab("lab orders")}
              />
            )}
          </MDBNav>
        </MDBBox>
        ...
        >
          <LeadSummary
            toggleEndInteraction={this.toggleEndInteraction}
            className=""
          />
          <MDBBox
            className="d-flex"
            style={{ flex: 1, overflow: "auto", flexDirection: "row" }}
          >
            <MDBBox
              className="d-flex mt-2 mr-2"
              style={{ flex: 1, overflow: "auto", flexDirection: "column" }}
            >
        ...
        {this.state.endInteractionVisible && (
          <EndInteraction
            history={this.props.history}
            toggle={this.toggleEndInteraction}
          />
        )}
        <MDBModal
          isOpen={this.state.unsavedNoteModalVisible}
          toggle={this.closeUnsavedNoteModal}
        >
          <MDBModalHeader>
            {localization.endInteraction.unsavedNoteModalHeader}
          </MDBModalHeader>
          <MDBModalBody>
        ...
        <MDBModal
          isOpen={this.props.twilio.interactionIncomingCallSID !== ""}
          centered
          toggle={() => {}}
          keyboard={false}
        >
          <MDBModalBody className="p-4 d-flex flex-column align-items-center">
            <h3>
              {localization.answerInteractionIncomingTitle.replace(
                "$",
                this.props.lead.details.first_name
              )}
            </h3>
            <MDBBtn
              className="button danger p-1 m-3"
              onClick={this.answerIncoming}
            >
              {localization.answerInteractionIncomingLabel}
            </MDBBtn>
          </MDBModalBody>
        </MDBModal>
